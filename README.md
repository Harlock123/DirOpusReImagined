...
- **Double-click any file to open it with the system default application** — executables run, images open in the image viewer, media plays, documents open in their editor. Windows uses the shell, macOS Launch Services, Linux the desktop's MIME associations
...
- **Syntax highlighting** for recognised source files — around 70 extensions across 38 languages (C#, Python, JS/TS, Rust, Go, SQL, YAML, XML/XAML, Markdown, shell and more), with line numbers
- Highlighting colours follow the app theme: **Solarized Light/Dark, Dracula and Nord** map to the matching syntax palette, so the preview never clashes with the rest of the window
- Files over 100 KB, and any type without a known language, fall back to plain text — tokenising runs on every cursor move, so the cap keeps arrow-key navigation responsive
- The window **remembers its size and position** between sessions, and a saved position is checked against the screens actually attached, so a window last closed on a monitor you no longer have still opens somewhere visible
- **Audio and video files** show their tags — title, artist, album, year, track, genre — plus duration, sample rate, channels and bitrate; AVI, Matroska and WebM video add resolution, frame rate and codecs
- **PDF documents** show an info card — version, page count, title, author, producer and dates — rather than a meaningless hex dump
- **Word, Excel and PowerPoint files** (and their OpenDocument equivalents) preview as their text — paragraphs, a cell grid, or a slide-by-slide outline
- **Archives** list their contents — entry count, uncompressed size, compression ratio and the first 200 entries — and are named by what they actually hold
...
- **Syntax-highlighted text previews.** Recognised source files are coloured and line-numbered, covering ~70 extensions across 38 languages, via [SyntaxColorizer](https://github.com/Harlock123/SyntaxColorizer). The palette follows the app theme — Solarized Light/Dark, Dracula and Nord map to the matching syntax theme exactly, and plain Light/Dark fall back to the Visual Studio palettes. Files over 100 KB, and types with no known language, stay plain text so that tokenising cannot slow down arrow-key navigation.
- **The preview window remembers its size and position** across sessions (`<PreviewWidth>` / `<PreviewHeight>` / `<PreviewX>` / `<PreviewY>`), rather than resetting each time it is toggled. A saved position is validated against the screens actually present before it is used, so a window last closed on a monitor that is no longer attached does not reopen off-screen.
- **Media tag previews.** MP3, FLAC, WAV, MP4/M4A, Ogg, AVI, Matroska and WebM files now preview as their tags and stream details — title, artist, album, year, track, genre, duration, sample rate, channels and bitrate. Read natively with no new dependency: ID3v2 (2.2/2.3/2.4) with an ID3v1 fallback, FLAC metadata blocks, ISO base-media atoms, RIFF chunks and Ogg Vorbis/Opus headers are each a short, stable header. Nothing decodes audio, so the cost does not scale with the length of the track, and MP3 duration uses the Xing/Info frame count when present rather than estimating from the file size. AVI additionally reports resolution, frame rate and the video/audio codecs, naming the common FourCCs (`FMP4`, `DX50`, `XVID` and friends all being MPEG-4 Part 2) while keeping the raw code. Matroska and WebM are read through their EBML tree — resolution, codecs, channels, sample rate and tags — and are told apart by their DocType, since the two share a magic number.
- **Office and OpenDocument previews.** `.docx`, `.xlsx`, `.pptx` and their `.odt`/`.ods`/`.odp` equivalents now preview as their content: Word paragraphs as text, spreadsheet cells as an aligned grid (shared strings resolved, gaps in the row preserved), and presentations as a slide-by-slide outline. No new dependency — these formats are ZIP containers of XML, both of which the app already reads, and the parts are fetched through the archive layer so the entry cache is shared. Parsing ignores XML namespaces and matches element names, which keeps it working across OOXML versions and between Office and OpenDocument.
- **Archive listings.** Selecting a ZIP, 7z, RAR, TAR or compressed tarball now shows what is inside it: entry and folder counts, total uncompressed size, how much the compression saved, and the first 200 entries with their sizes. Reuses the existing archive reader, so it inherits the streaming fallback that compression-wrapped tarballs need and shares its entry cache with browsing. Archives whose contents identify a specific format are named as such — Word/Excel/PowerPoint documents, OpenDocument files, EPUBs, JARs and APKs are all ZIP containers, and a `.tar.gz` is reported as a gzip-compressed TAR rather than the bare gzip stream its header claims.
- **Double-click opens any file with the OS default application.** Previously this was gated on a configurable extension list: on Windows only listed types opened, and on Linux nothing happened at all for an ordinary file, because that list was only consulted on Windows — a `.jpg` with normal permissions was simply ignored. Now every file is handed to the operating system, which already knows what can open it. On Linux this uses `xdg-open` even for files carrying an execute bit, so a double-click never runs a binary the desktop would rather open in an editor. Files inside an archive are extracted to a temp copy first (as external tool buttons already did); files on a cloud remote report that they must be copied locally first. A launcher that fails — no registered handler, or no `xdg-open` — now says so instead of failing silently.
- **PDF info cards.** A PDF now previews as a summary — PDF version, page count, encryption, and whatever its `/Info` dictionary holds (title, author, subject, creator, producer, created/modified dates) — instead of falling through to a hex dump. Metadata is read by scanning, with no native dependency: rendering a page would mean shipping a ~10 MB PDFium binary per platform inside every single-file executable. A document that keeps its metadata in a compressed object stream reports the fields readable in the clear and says so, rather than guessing.
...
- **Deleting on a network share no longer freezes and kills the app** — deleting a selection from a UNC share hung the window and then crashed the process with no error dialog. Three faults stacked up: network locations have **no Recycle Bin**, so every "Move to Trash" delete made a round of blocking network calls and then failed; the whole delete loop ran **on the UI thread**, so the window was frozen for the duration; and each failure fired an **unawaited modal dialog from inside that loop**, stacking one broken modal per item on top of an already-modal dialog until the process went down. Now: network locations are detected up front and you get a single "No Recycle Bin here — delete permanently?" confirmation (with Cancel) instead of a silent downgrade or a doomed trash attempt; the delete runs **off the UI thread** so the app stays responsive; and any failures are collected and reported in **one** summary dialog afterwards.
- **Delete builds cloud paths correctly** — deleting inside a `cloud://` remote on Windows spliced a backslash into the URI (`Path.Combine`), so the delete silently targeted a nonexistent key. Delete now uses the same scheme-aware path join as the rest of the app.
- **Crashes are no longer silent** — an exception escaping an `async void` event handler (which every button click handler is) killed the process outright: no dialog, no message, nothing to report. DORI now installs handlers for the Avalonia dispatcher, the app domain, and unobserved tasks at startup. A UI-thread fault is **recorded with a full stack trace and the app keeps running**, showing an "Unexpected error" dialog that names the log file; background and fatal faults are recorded on the way out. The delete dialog additionally wraps its own handler, so a fault there surfaces as a message rather than vanishing.
- **Where the crash log lives** — `%APPDATA%\dori\crash.log` on Windows, `~/Library/Application Support/dori/crash.log` on macOS, `~/.config/dori/crash.log` on Linux. Entries are appended with a timestamp and OS/architecture, so a crash can be reported after the fact.
